...
// Get Training Content with Redis Caching
app.get("/trainings", async (req, res) => {
  try {
    const { location } = req.query;
    const cacheKey = `trainings:${location || "all"}`;
    const cachedData = await redis.get(cacheKey);
    if (cachedData) return res.json(JSON.parse(cachedData));
    
    const trainings = await Training.find(location ? { location } : {});
    await redis.setex(cacheKey, 3600, JSON.stringify(trainings)); // Cache for 1 hour
...
app.post("/trainings", async (req, res) => {
  try {
    const { course_name, provider, location } = req.body;
    if (!course_name || !provider || !location) return res.status(400).json({ error: "All fields are required" });
    
    const training = new Training({ course_name, provider, location });
    await training.save();
    res.json({ message: "Training program added successfully!", training });
  } catch (error) {
    res.status(500).json({ error: "Failed to add training program", details: error.message });
  }
});
